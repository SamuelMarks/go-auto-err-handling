...
func NewInjector(pkg *packages.Package) *Injector {
	return &Injector{
		Fset: pkg.Fset,
		Pkg:  pkg,
	}
	...
	astutil.Apply(file, func(c *astutil.Cursor) bool {
		...
		if stmt, isStmt := node.(ast.Stmt); isStmt {
			if point, exists := pointMap[stmt]; exists {
				...
				newNodes, rewriteErr := i.generateRewrite(point, ctx.sig, ctx.decl)
				if rewriteErr != nil {
					err = rewriteErr
...
func (i *Injector) AddImport(file *ast.File, pkgPath string) bool {
	return imports.Add(i.Fset, file, pkgPath)
}

func (i *Injector) generateRewrite(point analysis.InjectionPoint, sig *types.Signature, decl *ast.FuncDecl) ([]ast.Stmt, error) {
	...
	retStmt, err := i.generateReturnStmt(sig)
	if err != nil {
		return nil, err
	}
	...
	assignStmt, err := i.generateAssignment(point)
	if err != nil {
		return nil, err
	}
...
func (i *Injector) generateReturnStmt(sig *types.Signature) (*ast.ReturnStmt, error) {
	...
	if sig != nil {
		...
		// How to detect?
		// We can check if last param is error.
		limit := sig.Results().Len()
		if limit > 0 {
			last := sig.Results().At(limit - 1)
			if i.isErrorType(last.Type()) {
				limit-- // Skip the error, we append "err" manually
			}
		}
		...
		for idx := 0; idx < limit; idx++ {
			...
			returnExprs = append(returnExprs, zero)
		}
	}

	returnExprs = append(returnExprs, &ast.Ident{Name: "err"})
	return &ast.ReturnStmt{Results: returnExprs}, nil
}

func (i *Injector) generateAssignment(point analysis.InjectionPoint) (ast.Stmt, error) {
	call := point.Call
