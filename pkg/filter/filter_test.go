...
func TestMatchesFile(t *testing.T) {
	...
	// Setup Temp Dir for real file interactions
	tmpDir := t.TempDir()
	realPath := filepath.Join(tmpDir, "real.go")
	_ = os.WriteFile(realPath, []byte("package p"), 0644)
	fReal := fset.AddFile(realPath, -1, 100)
	...
	tests := []struct {
		...
		{
			name:        "RealFile_NoGlobMatch_NotGenerated",
			globs:       nil,
			pos:         fReal.Pos(1),
			wantMatch:   false,
			expectError: false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := f.MatchesFile(fs, tt.pos)
			if tt.expectError {
				if err == nil {
					t.Error("MatchesFile() expected error, got nil")
				}
			} else {
				if err != nil {
...
func TestMatchesFile_Generated(t *testing.T) {
	...
	t.Run("StandardGenerated", func(t *testing.T) {
		...
		f := New(nil, nil)
		match, err := f.MatchesFile(fset, pos)
		if err != nil {
			t.Fatal(err)
		}
		if !match {
